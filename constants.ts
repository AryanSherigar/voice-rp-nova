...
const createNeonProtocolState = (): GameState => {
...
const createFantasyState = (): GameState => {
  const createdAt = Date.now();
...
const createHostileClientDeescalationState = (): GameState => {
  const createdAt = Date.now();
...
const createMedicalTriageCrisisState = (): GameState => {
  const createdAt = Date.now();
...
export const PREMADE_SCENARIOS: ScenarioTemplate[] = [
  {
    ...
    initialStateFactory: createHostileClientDeescalationState
  },
  {
